...
if APACHI
    ...
    if strcmp(computer,'MACI64') % for Jim's Mac
        if strcmp(annot_Team,'sandbox')
            matDir = '~/Data/sandbox/APACHI/AVA_valid/'; % jkr's - add your own as needed
        else
            if exist('~/Data/APACHI/AVA_valid','dir')
                matDir = '~/Data/APACHI/AVA_valid/'; % note: trailing '/' is required
            else
